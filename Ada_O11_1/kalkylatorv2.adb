...
procedure Kalkylator is
   ...
   begin
	 ...
	 loop
	    Put("Val: ");
	    Get(Selected_Letter);
	    exit when Selected_Letter >= 1 and Selection <= 4;
	    Put_Line("Felaktigt val!");
	 end loop;
	 
	 if Selected_Letter = 1 then
	    Hypothenuse_Program;
	 elsif 
	   Selected_Letter = 2 then
	    Angle_Program;
	 elsif 
	   Selected_Letter = 3 then
	    Factorial_Program;
	 elsif 
	   Selected_Letter = 4 then
	    Put("Ha en bra dag!");
	    exit;
	 end if;
      end loop;
